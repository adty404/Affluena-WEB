...
export function TransactionListPage() {
  const income = mockTransactions.filter((tx) => tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0);
  const expenses = mockTransactions.filter((tx) => tx.type === 'expense').reduce((sum, tx) => sum + tx.amount, 0);
  const columns = [
    { key: 'title', header: 'Transaction', render: (tx: Transaction) => <div><strong>{tx.title}</strong><span className="table-subtitle">{tx.note}</span></div> },
    { key: 'type', header: 'Type', render: (tx: Transaction) => <Badge tone={tx.type === 'income' ? 'green' : tx.type === 'expense' ? 'red' : tx.type === 'transfer' ? 'blue' : 'orange'}>{transactionTypeLabels[tx.type]}</Badge> },
    { key: 'wallet', header: 'Wallet', render: (tx: Transaction) => tx.destinationWalletName ? `${tx.walletName} → ${tx.destinationWalletName}` : tx.walletName },
    { key: 'category', header: 'Category', render: (tx: Transaction) => tx.categoryName ?? '—' },
    { key: 'amount', header: 'Amount', align: 'right' as const, render: (tx: Transaction) => <Amount value={tx.amount} variant={tx.type === 'income' ? 'income' : tx.type === 'expense' ? 'expense' : 'neutral'} /> },
    { key: 'action', header: 'Action', render: (tx: Transaction) => <div className="inline-actions"><Button size="small" to={`/transactions/${tx.id}`}>View</Button><Button size="small" to={`/transactions/${tx.id}/edit`}>Edit</Button></div> },
  ];
    ...
    <AppLayout title="Transactions" description="Income, expense, transfer, adjustment, split bill, and quick entry.">
      <div className="dashboard-page grid-stack">
        ...
        <section className="stat-grid">
          ...
          <Card className="stat-card blue"><span>Transactions</span><strong>{mockTransactions.length}</strong><small>Posted records</small></Card>
        ...
        <section className="dashboard-grid">
          <Card className="panel-card">
            ...
            <div className="transaction-list">{mockTransactions.slice(0, 4).map((tx) => <TransactionItem key={tx.id} transaction={tx} />)}</div>
          </Card>
          <Card className="panel-card">
        ...
        <Card className="panel-card">
          ...
          <DataTable columns={columns} data={mockTransactions} getRowKey={(tx) => tx.id} />
        </Card>
      </div>
    </AppLayout>
  );
