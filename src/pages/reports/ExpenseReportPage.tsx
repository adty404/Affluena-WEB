...
export function ExpenseReportPage() {
  ...
  const handleExport = () => {
    if (!data || (rows.length === 0 && metrics.length === 0)) {
      showToast('Tidak ada data pengeluaran untuk diekspor pada periode ini.');
      return;
    }
    exportReportCsv('Laporan Pengeluaran', month, data);
    showToast('Laporan pengeluaran berhasil diekspor sebagai CSV.');
  };

  return (
    <AppLayout title="Laporan Pengeluaran" description="Sebaran pengeluaran per kategori, dompet, dan tren.">
          ...
          <>
            ...
            <Card className="panel-card">
              ...
              <DataTable<ReportRow>
                ...
                columns={[
                  { key: 'name', header: 'Nama', render: (row) => <div className="table-title"><span className={`mini-icon ${row.status === 'critical' ? 'danger' : row.status === 'watch' ? 'warning' : row.status === 'growth' ? 'info' : 'safe'}`}><AppIcon name="shopping" /></span><strong>{row.name}</strong><small>{row.category}</small></div> },
            ...
            <PageMetaStrip
              title="Status laporan pengeluaran"
              items={[
