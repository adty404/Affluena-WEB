...
export function CashflowReportPage() {
  ...
  const { data, isLoading, isError } = useCashflowReport(month);
  ...
  const handleExport = () => {
    if (!data || (rows.length === 0 && metrics.length === 0)) {
      showToast('Tidak ada data arus kas untuk diekspor pada periode ini.');
      return;
    }
    exportReportCsv('Laporan Arus Kas', month, data);
    showToast('Laporan arus kas berhasil diekspor sebagai CSV.');
  };

  return (
    <AppLayout title="Laporan Arus Kas" description="Pergerakan bersih semua dompet per periode.">
      <div className="dashboard-page grid-stack">
        <section className="app-hero-card dashboard-hero">
          <div>
            <Badge>Arus Kas · {monthLabel}</Badge>
            <h2>Arus kas bersih terlihat per minggu agar surplus dan defisit cepat terbaca.</h2>
            <p>Pemasukan dikurangi pengeluaran, dihitung per periode dan dompet.</p>
          </div>
        ...
        <section className="report-filter-bar">
          ...
          <div className="report-meta-chip"><span>Kategori</span><strong>Semua jenis transaksi</strong></div>
        ...
        {isLoading ? (
          ...
          <Card className="panel-card"><EmptyState icon={<AppIcon name="chart" />} title="Belum ada data laporan" description={`Belum ada data arus kas untuk ${monthLabel}.`} /></Card>
        ) : (
          <>
            ...
            <Card className="panel-card">
              ...
              <DataTable<ReportRow>
                ...
                columns={[
                  { key: 'name', header: 'Nama', render: (row) => <div className="table-title"><span className={`mini-icon ${row.status === 'critical' ? 'danger' : row.status === 'watch' ? 'warning' : row.status === 'growth' ? 'info' : 'safe'}`}><AppIcon name="chart" /></span><strong>{row.name}</strong><small>{row.category}</small></div> },
