...
export function CashflowReportPage() {
  ...
  const handleExport = () => {
    if (!data || (rows.length === 0 && metrics.length === 0)) {
      showToast('No cashflow data to export for this period.');
      return;
    }
    exportReportCsv('Cashflow Report', month, data);
    showToast('Cashflow report exported as CSV.');
  };

  return (
    <AppLayout title="Cashflow Report" description="Net movement by period across all wallets.">
      <div className="dashboard-page grid-stack">
        <section className="app-hero-card dashboard-hero">
          <div>
            <Badge>● Cashflow · {monthLabel}</Badge>
            <h2>Cashflow bersih terlihat per minggu agar surplus dan defisit cepat terbaca.</h2>
            <p>Menghitung income minus expense berdasarkan periode dan wallet.</p>
          </div>
        ...
        <section className="report-filter-bar">
          ...
          <Card className="filter-card"><span>Category</span><strong>All transaction types</strong></Card>
        ...
        {isLoading ? (
          ...
          <Card className="panel-card"><div className="empty-state"><p>No cashflow data for {monthLabel}.</p></div></Card>
        ) : (
          <>
            ...
            <Card className="panel-card">
              ...
              <DataTable<ReportRow>
                ...
                columns={[
                  { key: 'name', header: 'Name', render: (row) => <div className="table-title"><span className={`mini-icon ${row.status === 'critical' ? 'danger' : row.status === 'watch' ? 'warning' : row.status === 'growth' ? 'info' : 'safe'}`}><AppIcon name="chart" /></span><strong>{row.name}</strong><small>{row.category}</small></div> },
