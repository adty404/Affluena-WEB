...
export function GoalReportPage() {
  ...
  const handleExport = () => {
    if (!data || (rows.length === 0 && metrics.length === 0)) {
      showToast('No goal data to export for this period.');
      return;
    }
    exportReportCsv('Goal Report', month, data);
    showToast('Goal report exported as CSV.');
  };

  return (
    <AppLayout title="Goal Report" description="Goal progress, contribution health, and shared goal status.">
      <div className="dashboard-page grid-stack">
        <section className="app-hero-card dashboard-hero">
          <div>
            <Badge>● Goals · {monthLabel}</Badge>
            <h2>Goal report memperlihatkan target, saved amount, dan progress setiap goal.</h2>
            <p>Termasuk private goal dan shared goal dengan member contribution.</p>
          </div>
        ...
        <section className="report-filter-bar">
          ...
          <Card className="filter-card"><span>Category</span><strong>Financial goals</strong></Card>
        ...
        {isLoading ? (
          ...
          <Card className="panel-card"><div className="empty-state"><p>No goal data for {monthLabel}.</p></div></Card>
        ) : (
          <>
            ...
            <Card className="panel-card">
              ...
              <DataTable<ReportRow>
                ...
                columns={[
                  { key: 'name', header: 'Name', render: (row) => <div className="table-title"><span className={`mini-icon ${row.status === 'critical' ? 'danger' : row.status === 'watch' ? 'warning' : row.status === 'growth' ? 'info' : 'safe'}`}><AppIcon name="goal" /></span><strong>{row.name}</strong><small>{row.category}</small></div> },
