...
export function DebtReportPage() {
  ...
  const handleExport = () => {
    if (!data || (rows.length === 0 && metrics.length === 0)) {
      showToast('No debt data to export for this period.');
      return;
    }
    exportReportCsv('Debt Report', month, data);
    showToast('Debt report exported as CSV.');
  };
    ...
    <AppLayout title="Debt Report" description="Payable, receivable, due risk, and collection health.">
      <div className="dashboard-page grid-stack">
        ...
        {isLoading ? (
          ...
          <Card className="panel-card"><div className="empty-state"><p>No debt data for {monthLabel}.</p></div></Card>
        ) : (
          <>
