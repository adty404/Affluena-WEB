...
export function DebtReportPage() {
  ...
  const { data, isLoading, isError } = useDebtReport(month);
  ...
  const handleExport = () => {
    if (!data || (rows.length === 0 && metrics.length === 0)) {
      showToast('Tidak ada data utang untuk diekspor pada periode ini.');
      return;
    }
    exportReportCsv('Laporan Utang', month, data);
    showToast('Laporan utang berhasil diekspor sebagai CSV.');
  };

  return (
    <AppLayout title="Laporan Utang" description="Utang, piutang, risiko jatuh tempo, dan penagihan.">
      <div className="dashboard-page grid-stack">
        <section className="app-hero-card dashboard-hero">
          <div>
            <Badge>Utang · {monthLabel}</Badge>
            <h2>Ukur utang dan piutang berjalan yang perlu ditindaklanjuti.</h2>
            <p>Bantu kamu menentukan prioritas bayar dan tagih.</p>
          </div>
        ...
        <section className="report-filter-bar">
          ...
          <div className="report-meta-chip"><span>Kategori</span><strong>Utang & piutang</strong></div>
        ...
        {isLoading ? (
          ...
          <Card className="panel-card"><EmptyState icon={<AppIcon name="chart" />} title="Belum ada data laporan" description={`Belum ada data utang untuk ${monthLabel}.`} /></Card>
        ) : (
          <>
            ...
            <Card className="panel-card">
              ...
              <DataTable<ReportRow>
                ...
                columns={[
                  { key: 'name', header: 'Nama', render: (row) => <div className="table-title"><span className={`mini-icon ${row.status === 'critical' ? 'danger' : row.status === 'watch' ? 'warning' : row.status === 'growth' ? 'info' : 'safe'}`}><AppIcon name="debt" /></span><strong>{row.name}</strong><small>{row.category}</small></div> },
                  { key: 'wallet', header: 'Dompet', render: (row) => row.wallet },
                  { key: 'amount', header: 'Saat Ini (Rp)', align: 'right', render: (row) => <Amount value={row.amount_minor} type="expense" /> },
