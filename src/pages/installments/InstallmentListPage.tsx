    ...
    <AppLayout title={NAV.cicilan} description="Pantau tenor cicilan, sisa pokok, dan jadwal tagihan bulanan.">
      <div className="dashboard-page grid-stack">
        <section className="app-hero-card dashboard-hero">
          <div><Badge className="dark">{NAV.cicilan}</Badge><h2>Pantau cicilan tetap, sisa tenor, dan pembayaran berikutnya.</h2><p>Setiap cicilan punya dompet pembayaran, nominal bulanan, tenor, dan pengingat yang jelas.</p></div>
        ...
        {error ? (
          ...
          <div className="loading-state">Memuat cicilan...</div>
              ...
              {installments.map((item) => {
                ...
                return (
                  <FinanceOverviewCard
                    key={item.id}
                    title={item.name}
                    subtitle={`${paidCount} dari ${item.tenor_months} bulan · ${walletLabel(walletNameById, item.wallet_id)}`}
                    icon="installment"
                    iconTone="info"
                    ...
                    actions={<><Button to={`/installments/${item.id}/pay`} size="small" variant="primary"><AppIcon name="pay" /> Bayar</Button><Button size="small" variant="danger" onClick={() => setTarget(item)} aria-label={`Hapus cicilan ${item.name}`}><AppIcon name="delete" /> Hapus</Button></>}
                  />
                );
              })}
            </section>
            ...
            <Card className="panel-card">
              ...
              <DataTable<Installment>
                ...
                columns={[
                  ...
                  { key: 'action', header: 'Aksi', render: (item) => <div className="inline-actions"><Button to={`/installments/${item.id}/pay`} size="small">Bayar</Button><Button size="small" variant="danger" onClick={() => setTarget(item)} aria-label={`Hapus cicilan ${item.name}`}><AppIcon name="delete" /></Button></div> },
                ]}
              />
            </Card>
          </>
